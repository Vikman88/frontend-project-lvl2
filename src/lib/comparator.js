...
const makeNode = (key, selector, value, newValue) => ({ key, selector, value, newValue });

const objSelectNode = {
  [node]: (key, value, newValue) => makeNode(key, node, compare(value, newValue)),
  [leaf]: (key, value, newValue) => makeNode(key, leaf, value),
...
const makeAST = (acc, key, obj1, obj2) => {
  ...
  const node = objSelectNode[selector](key, value, newValue);
  return [...acc, node];
};
